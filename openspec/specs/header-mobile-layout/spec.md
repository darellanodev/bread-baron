...
The system SHALL hide the entire HeaderTitle block (icon + text) on viewports below 768px (below `md` breakpoint).
...
The system SHALL remove the `min-w-[84px]` constraint from the pause button on viewports below 768px, allowing it to shrink to icon-only size.
...
The system SHALL reduce the SettingsMenu trigger button from 40px (`size-10`) to 32px (`size-8`) on viewports below 768px.
...
The system SHALL reduce Stat pill value font size from `text-sm` to `text-xs` on viewports below 768px.
...
The system SHALL reduce the gap between HeaderStats and HeaderActions from `gap-8` to `gap-3` on viewports below 768px.
