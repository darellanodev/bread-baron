...
export function BakeryOven({ compact = false }: BakeryOvenProps) {
  ...
  return (
    <div className="relative group flex-none">
      ...
      <div className={`${compact ? 'w-20 h-20 border-4 rounded-lg' : 'size-64 border-8 rounded-xl'} bg-gradient-to-br from-brownDark to-brownDarker border-brownDarkest shadow-2xl flex items-center justify-center relative`}>
        {isPaused ? (
          <div className={`${compact ? 'w-12 h-10 border-2' : 'w-48 h-36 border-4'} bg-brownDeep rounded-lg border-primary/30 flex items-center justify-center`}>
